    ...
    <div>
      <section className="bg-hero px-20 pt-15 flex flex-col gap-10">
        <div className="flex flex-col gap-6 items-center">
          ...
          <div className="flex flex-col gap-5 items-center relative py-5">
            <h1 className="font-anek font-semibold text-white max-w-[40ch] text-5xl text-center leading-15">
              Turn Your Event Conversations into Measurable Outcomes | In Real
              Time
            </h1>
            <p className="font-inter text-white text-lg text-center max-w-[70ch]">
      ...
      <section className="flex flex-col gap-16 py-30 px-20 bg-feature bg-neutral800">
        ...
        <div className="flex flex-col md:flex-row gap-5">
          {features.map(({ title, description }, i) => (
            <div
              key={title}
              className={`flex flex-col w-full border border-white/10 rounded-3xl py-12.5 px-7.5 gap-10 ${
      ...
      <section className="flex flex-col gap-16 py-30 px-20 bg-feature items-center">
        <div className="relative py-5 px-10 flex flex-col gap-5 items-center max-w-2xl">
          ...
          <p className="text-lg text-pink font-inter text-center upper">
            ...
            {resultOne.map(({ img, title, description }) => (
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <p className="text-white/80 font-inter text-sm font-light max-w-[30ch]">
                    {description}
                  </p>
                </div>
              </div>
      ...
      <section className="bg-blue150 px-20 py-20 flex flex-col md:flex-row md:items-center md:gap-10">
        ...
        <div className="flex flex-col gap-6">
          <h5 className="uppercase font-inter text-lg font-semibold text-pink">
            WHO WE BUILT THIS FOR
          </h5>
          <h4 className="text-3xl font-bold font-anek text-white">
          ...
          <div className="flex flex-col gap-4">
            {designs.map((design) => (
              ...
              >
                <div className="shrink-0 w-12 h-12 rounded-md bg-white flex items-center justify-center">
                  <FaRegTrashAlt className="w-6 h-6 text-black" />
                </div>
                <p className="text-white/80 font-inter text-lg">{design}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="flex flex-col gap-16 py-30 px-20 bg-feature items-center">
        <div className="relative py-5 px-10 flex flex-col gap-2 items-center max-w-3xl">
          <p className="text-lg text-pink font-inter font-bold text-center uppercase">
            HOW IT WORKS
          </p>
        ...
        <div className="flex flex-col gap-7 md:gap-5 md:flex-row md:items-center">
            {
                works.map((work,i) => <div key={work} className={`border border-white/30 rounded-2xl py-12.5 px-7.5 flex flex-col gap-10 self-stretch ${
                    i === 0 ? 'bg-cyan' :
                    i === 1 ? 'bg-pink' :
                    i === 2 ? 'bg-brown700' :
                    'bg-blue700'
                    ...
                    <p className="text-2xl font-bold text-white font-anek">{work}</p>
                </div>)
            }
      ...
      <section className="flex py-20 px-20 bg-blue150 items-center justify-center">
        <div className="flex flex-col md:flex-row md:gap-10 md:items-end w-full md:px-30 md:pb-30 bg-cta">
            <h4 className="text-3xl font-bold font-anek text-white max-w-[25ch] basis-1/2 md:pb-40">We’re ready to help you. Contact us now.</h4>
