...
function SocialmediaFeed() {
  return (
    <section className="space-y-6">
        ...
        <h1 className='text-white text-2xl font-semibold font-lexend'>SOCIAL MEDIA FEED</h1>
        <p className='text-white font-lexend font-light text-xs'>Real-time social media mentions, trends and engagement around Invest Lagos 3.0</p>
      </div>
      ...
      <section className="grid grid-cols-12 gap-5">
        <div className='border border-white/55 rounded-2xl py-5 px-7.5 flex flex-col gap-6 col-span-5 '>
                <h4 className='font-dmSans text-white font-medium text-base uppercase'>SENTIMENT BREAKDOWN</h4>
                    <DonutChart small
                    data={[
        ...
        <div className='flex flex-col gap-4'>
            {
                sectorOne.map(({icon, percent}, i) => <div key={i} className='grid grid-cols-12 gap-10 place-content-between'>
                    <div className="w-6 h-6 rounded-md bg-white flex items-center justify-center col-span-2">{icon}</div>
                    ...
                    <p className='col-span-2 text-white font-dmSans text-sm flex items-center justify-center'>{percent}%</p>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className='flex items-center justify-between gap-3'>
            <h4 className='text-white/55 font-medium uppercase text-base font-lexend'>LATEST ADDITIONS</h4>
            <button className='text-cyan font-semibold font-lexend text-base'>View all themes</button>
        </div>
        <div className='flex flex-col gap-6'>
            {
                additions.map(({title, value}) => <div key={title} className='flex items-center justify-between gap-3'>
      ...
      <div className='border border-white rounded-2xl px-7.5 py-2.5 flex flex-col gap-7.5 lg:col-span-7'>
        <div className='flex items-center justify-between gap-3'>
            <h4 className='text-white font-medium uppercase text-base font-lexend'>TRENDING HASHTAGS</h4>
            <button className='text-cyan font-semibold font-lexend text-base'>View all hashtags</button>
        </div>
        <div className='flex flex-col gap-6'>
            {
                hashtags.map(({title, value}) => <div key={title} className='flex items-center justify-between gap-3'>
