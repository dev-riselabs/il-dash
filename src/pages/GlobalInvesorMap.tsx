...
const investors =[
    {name : 'Africa Finance Corporation', sector: 'Infrastructure', time: '2m ago'},
    {name : 'Plug and Play Ventures', sector: 'Technology', time: '5m ago'},
    {name : 'European Investment Bank', sector: 'Infrastructure', time: '8m ago'},
...
function GlobalInvesorMap() {
  return (
    <section className="space-y-6">
        ...
        <h1 className='text-white text-2xl font-semibold font-lexend'>GLOBAL INVESTOR MAP</h1>
        <p className='text-white font-lexend font-light text-xs'>Real-time view of investor participation and interest by region </p>
      </div>
      ...
      <div className='flex items-center justify-between gap-3'>
                  <h4 className='text-white font-medium uppercase text-base font-lexend'>TOP COUNTRIES BY INVESTOR COUNT</h4>
        ...
        <div className="flex lg:items-center flex-col lg:flex-row gap-1.5">
            {
                countries.map(({name, percent, total}, i) => <div className="border border-white/30 rounded-xl py-3.75 px-5 gap-8 flex-1 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <span className="text-white text-sm font-dmSans font-semibold">{i + 1}</span>
                        <img src={`${
                            i === 0 ? '/ng-flag.png': 
                            i === 1 ? '/uk-flag.png' :
                            i === 2 ? '/usa-flag.png' :
                            '/germany-flag-48866.png'
                        }`} alt="" className="w-12 h-12 rounded-md"/>
                    </div>
                    <div className="flex flex-col gap-2">
                        <h5 className="text-white text-sm font-dmSans font-semibold">{name}</h5>
                        <div className="flex items-center gap-4">
                            <span className="text-white text-xs font-dmSans font-light">{total}</span>
                            <span className="text-white text-xs font-dmSans font-light">{percent}</span>
                        </div>
                    </div>
                </div>)
            }
      ...
      <section className='grid grid-cols-1 lg:grid-cols-12 gap-6'>
                    ...
                    <div className='flex items-center justify-between gap-3'>
                  <h4 className='text-white font-medium uppercase text-base font-lexend'>INVESTOR SUMMARY</h4>
                  <button className='text-cyan font-semibold font-lexend text-base'>View full report</button>
              </div>
                    <DonutChart
              data={[
            ...
            />
                    ...
                    <div className='border border-white/55 rounded-2xl py-5 px-7.5 flex flex-col gap-6 lg:col-span-6'>
                    <div className='flex items-center justify-between gap-3'>
                  <h4 className='text-white font-medium uppercase text-base font-lexend'>TOP SECTORS OF INTREST</h4>
                  <button className='text-cyan font-semibold font-lexend text-base'>View all</button>
              </div>
                  ...
                  <div className='flex items-center justify-between gap-3'>
                  <h4 className='text-white font-medium uppercase text-base font-lexend'>RECENT INVESTORS</h4>
                  <button className='text-cyan font-semibold font-lexend text-base'>View all</button>
              </div>

              <div className="flex flex-col gap-5">
                {
                    investors.map(({name, sector, time}) => <div key={name} className="flex items-center justify-between">
                     <div className="flex items-center gap-2">
                        <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center"><FaYelp className="w-5 h-5 text-black"/></div>
                        <div className="flex flex-col gap-2">
                            <h6 className="text-white font-semibold text-sm font-dmSans">{name}</h6>
                            <span className="text-white font-light text-xs font-dmSans">{sector}</span>
                        </div>
                     </div>
                     <span className="text-slate100 font-semibold text-sm font-dmSans">{time}</span>
                    </div>)
                }
              </div>
                  </div>
