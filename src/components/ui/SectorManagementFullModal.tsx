...
interface SectorManagementFullModalProps {
  isOpen: boolean
  onClose: () => void
}

export function SectorManagementFullModal({ isOpen, onClose }: SectorManagementFullModalProps) {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-slate-950 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-white/20">
        <div className="sticky top-0 bg-slate-950 border-b border-white/20 flex items-center justify-between p-6">
          <h2 className="text-xl font-semibold text-white font-lexend">Manage Sectors</h2>
          <button
            onClick={onClose}
