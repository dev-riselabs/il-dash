...
export function fmtNaira(value: number | null | undefined): string {
  ...
  if (abs >= 1_000_000_000_000)
    return `${NAIRA}${(value / 1_000_000_000_000).toFixed(1)}T`;

  if (abs >= 1_000_000_000)
    return `${NAIRA}${(value / 1_000_000_000).toFixed(1)}B`;

  if (abs >= 1_000_000) return `${NAIRA}${(value / 1_000_000).toFixed(1)}M`;
...
export function fmtRelative(iso: string | null | undefined): string {
  ...
  if (abs < hr)
    return `${Math.round(abs / min)}m ${diff >= 0 ? "ago" : "from now"}`;
  if (abs < day)
